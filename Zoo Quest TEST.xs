...
code("xTowerDamage = xInitAddInt(dPlayerData, \"towerdmg\", 1);");
...
code("}");
code("bool rayCollision(vector start = vector(0,0,0), vector dir = vector(1,0,0),");
...
code("int boomID = 0;");
code("pos = kbGetBlockPosition(\"\"+xGetInt(dMissiles, xUnitID));");
code("if((trGetTerrainType(xsVectorGetX(pos)/2, xsVectorGetZ(pos)/2) == 0) && (trGetTerrainSubType(xsVectorGetX(pos)/2,");
code("xsVectorGetZ(pos)/2) == 1)){");
code("xUnitSelect(dMissiles, xUnitID);");
code("trUnitDestroy();");
code("xFreeDatabaseBlock(dMissiles);");
code("}");
...
code("xSetInt(dPlayerData, xTowerDamage, xGetInt(dPlayerData, xTowerDamage)+xGetFloat(dFreeRelics, xRelicStat));");
code("}");
code("}");
code("if(apply == false){");
...
code("closest = 10000;");
code("closestid = 0;");
code("for(a=0 ; < xGetDatabaseCount(dPoachers)){");
code("xDatabaseNext(dPoachers);");
code("dir = kbGetBlockPosition(\"\"+xGetInt(dPoachers, xUnitID));");
code("xUnitSelect(dPoachers, xUnitID);");
code("if(trUnitDead() == false){");
...
code("closest = 10000;");
code("closestid = 0;");
code("for(a=0 ; < xGetDatabaseCount(dTowers)){");
...
code("}");
code("closevector = kbGetBlockPosition(\"\"+closestid);");
code("xsSetContextPlayer(xGetInt(dTowers, xOwner));");
code("dest = kbGetBlockPosition(\"\"+trGetUnitScenarioNameNumber(kbUnitGetTargetUnitID(kbGetBlockID(\"\"+closestid))));");
code("xsSetContextPlayer(0);");
code("dir = xsVectorNormalize(dest-closevector);");
code("IGUnit = true;");
...
code("xSetInt(dMissiles, xOwner, cNumberNonGaiaPlayers);");
...
code("}");
code("void cameraLookAt(vector dest = vector(0.0, 0.0, 0.0), float distance = 0.0, float heading = 0.0,");
code("float tilt = 0.0, float verticalOffset = 0.0){");
...
code("}");
code("void deployCluster(float posX = 0.0, float posZ = 0.0, string unit = \"\", int p = 0, int count = 0,");
code("float radius = 0.0, bool forceOnMap = false){");
code("for(i = 0; < count){");
...
code("paintUnit(\"GrassDirt50\", \"Hoplite\", 0, 0.005);");
...
code("trSetCivAndCulture(0, 0, 0);");
code("clearMap(\"GrassB\", 5.0);");
code("int centrePosX = randomInt(toTiles(0.45), toTiles(0.55));");
code("int centrePosZ = randomInt(toTiles(0.45), toTiles(0.55));");
code("int ABORT = 0;");
code("smooth(4);");
code("vector tileForStart = vector(64,0,64);");
code("int StartTileX = xsVectorGetX(tileForStart);");
code("int StartTileZ = xsVectorGetZ(tileForStart);");
code("float StartHeight = 8;");
code("float StartMetreX = StartTileX*2+1;");
...
code("trVectorQuestVarSet(\"dir\", xsVectorSet(11, 0, 0));");
code("trVectorQuestVarSet(\"CentreMap\", xsVectorSet(StartMetreX, 0, StartMetreZ));");
code("paintCircleHeight2(StartTileX, StartTileZ, 8, \"GrassDirt25\", 5);");
code("float baseCos = xsCos(6.283185 / (cNumberNonGaiaPlayers-1));");
code("float baseSin = xsSin(6.283185 / (cNumberNonGaiaPlayers-1));");
code("int heading = 90;");
code("for(p=1; < cNumberNonGaiaPlayers) {");
...
code("case 0:");
code("{");
code("debugLog(\"NO CONSEQUENCE SET\");");
code("}");
code("case 1:");
code("{");
...
code("trSetCounterDisplay(\"<color={PlayerColor(0)}>Votes to skip: \" + 1*trQuestVarGet(\"SkipVotes\"));");
code("}");
code("case 4:");
code("{");
code("xSetPointer(dPlayerData, p);");
...
code("trChatSendToPlayer(0, p, \"Debug TP you should be player \" + xGetPointer(dPlayerData));");
code("if(Stage == 1){");
...
code("ActionChoice = 0;");
code("unitTransform(\"\"+YesChoiceUnitName + \" Hero\", \"Cinematic Block\");");
code("unitTransform(\"\"+NoChoiceUnitName + \" Hero\", \"Cinematic Block\");");
code("if(trCurrentPlayer() == p){");
code("if(Stage == 1){");
code("uiZoomToProto(\"\"+GazelleProto);");
code("}");
...
code("active");
code("{");
code("PlayerCycle = PlayerCycle+1;");
code("if(PlayerCycle > cNumberNonGaiaPlayers){");
code("PlayerCycle = 1;");
code("}");
code("int p = PlayerCycle;");
...
code("trShowImageDialog(\"icons\icon class harmless animal\", MapName + \" by Yeebaagooon\");");
...
code("modifyProtounitAbsolute(\"Tower\", p, 31, 1);");
...
code("inactive");
code("{");
code("if(PlayersActive == 1*trQuestVarGet(\"PlayersDoneTutorial\")){");
code("xsDisableSelf();");
...
code("}");
code("}");
code("rule DeerAllDead");
code("inactive");
code("minInterval 5");
code("{");
code("if((Stage == 1) && (PlayersDead == PlayersActive)){");
...
code("}");
code("PlayersMinigaming = PlayersMinigaming-1;");
code("trChatSend(0, \"Players MG\" + PlayersMinigaming);");
code("xSetBool(dPlayerData, xStopDeath, false);");
code("}");
code("}");
code("}");
...
code("if (trUnitIsSelected()) {");
code("uiClearSelection();");
code("trMessageSetText(\"Purchase \" + xGetString(dShop, xPower) + \" - Cost: \" + xGetInt(dShop, xCost), 6000);");
code("}");
code("trUnitSelectClear();");
code("}");
code("}");
...
code("}");
code("else{");
code("trMessageSetText(\"Sink 6 tiles into lava while remaining alive yourself!\", 8000);");
code("trCounterAddTime(\"cdgoatminigame\", 120,0,\"<color={PlayerColor(2)}>Minigame time remaining\", 37);");
...
code("ColouredIconChat(\"0.0,0.8,0.2\", \"icons\world column icon 64\", \"Lower tiles by interracting with columns.\");");
code("ColouredChat(\"0.0,0.8,0.2\", \"They will lower two squares.\");");
code("ColouredChat(\"0.0,0.8,0.2\", \"You get a warning before the tile becomes lava.\");");
code("ColouredChat(\"0.0,0.8,0.2\", \"When this happens, you won't be able to use the column!\");");
code("}");
code("for(x = xGetDatabaseCount(dInterractables); > 0){");
code("xDatabaseNext(dInterractables);");
code("if(xGetInt(dInterractables, xType) == 2){");
...
code("trUnitSelectClear();");
code("trUnitSelect(\"\"+temp);");
code("trUnitChangeProtoUnit(\"Hoplite\");");
code("trUnitSelectClear();");
code("trUnitSelect(\"\"+temp);");
...
code("xSetFloat(dHeldRelics, xRelicStat, xGetFloat(dFreeRelics, xRelicStat));");
...
code("ColouredChatToPlayer(dropper, \"1,0.2,0\", relicName(xGetInt(dFreeRelics, xUnitID)) + \" dropped\");");
code("trUnitChangeProtoUnit(\"Relic\");");
code("xUnitSelect(dHeldRelics, xUnitID);");
code("xAddDatabaseBlock(dFreeRelics, true);");
code("xUnitSelect(dHeldRelics, xUnitID);");
...
code("xSetFloat(dFreeRelics, xRelicStat, xGetFloat(dHeldRelics, xRelicStat));");
code("xSetInt(dFreeRelics, xSFXID, 1*trQuestVarGet(\"SFXUnit\"));");
...
code("if(TutorialMode == true){");
code("ForceRelic(1*trQuestVarGet(\"ArmoryP\"+p),1,1);");
code("if(trCurrentPlayer() == p){");
code("npcDiag(16);");
code("trCounterAbort(\"cdtutorial\");");
...
code("}");
code("}");
code("if((xGetInt(dPlayerData, xTowerDamage) > 1) && (1*trQuestVarGet(\"P\"+p+\"FountainMsg\") == 1)){");
code("trQuestVarSet(\"P\"+p+\"FountainMsg\", 2);");
code("if(trCurrentPlayer() == p){");
code("npcDiag(17);");
code("trCounterAbort(\"cdtutorial\");");
...
code("ColouredChat(\"0.0,0.8,0.2\", \"You will be hunted down shortly.\");");
code("ColouredChat(\"0.0,0.8,0.2\", \"Defend yourselves!\");");
code("xsEnableRule(\"PlayMusic\");");
code("PlayersDead = 0;");
code("timediff = trTimeMS();");
code("timelast = trTimeMS();");
...
code("modifyProtounitAbsolute(\"Tower\", p, 4, 10);");
code("modifyProtounitAbsolute(\"Armory\", p, 10, 1);");
code("modifyProtounitAbsolute(\"Armory\", p, 4, 60);");
code("}");
